    ...
    this.setRecommendations = function(){
        ...
        var finalRecommendations=[];
         for(var userids=0;userids<data.length;userids++){
            ...
            transactionModel.find({userId:phoneNumber,storeId:'1111',transactionDate:{$gt:date}}).exec(function(err,data){
           ...
           // console.log(JSON.stringify(map));
            ...
            ArrivalModel.find({itemCategory:key,itemSubCategory:{$in:value},storeId:'1111'},function(err,data){
                /* console.log('2 is'+JSON.stringify(data));*/
                for(var k=0;k<data.length;k++){
                itemIDs.push(data[k].itemId);
                }
            });
            discountModel.find({itemCategory:key,itemSubCategory:{$in:value},storeId:'1111'},function(err, data) {
                ...
                if(data!=null && data.length>0){
                    ...
                    function(err, data) {
                        ...
                        console.log('item ids is'+itemIDs);
                         InventoryModel.find({itemId:{$in:itemIDs},storeId:'1111'},function(err, data) {
